...
func (a *actorprocess) Receive(ctx actor.Context) {
	...
	case *MsgData:
		if err := func() error {
			...
			funcSend := func(sendData *processedData) {
				if sendData.distance*0.90 > float64(a.distanceMin) {
				...
				if time.Since(sendData.timeStamp) >= a.timeout {
					a.lastSendFrames[gtype] = result
					ctx.Send(ctx.Self(), &MsgSendFrame{
						Topic: topicEventGPS,
						Data:  sendData,
			...
			if gtype != GPGGA.Value() {
				if v1, ok := a.lastFrames[GPGGA.Value()]; ok && time.Since(v1.timeStamp) < 60*time.Second {
					if v2, ok := a.lastSendFrames[GPGGA.Value()]; ok && result.timeStamp.Sub(v2.timeStamp) < 1*time.Second {
						a.lastSendFrames[gtype] = result
						ctx.Send(ctx.Self(), &MsgSendFrame{
							Topic: topicEventGPS,
							Data:  result,
				...
				for _, v := range a.lastFrames {
					if result.timeStamp.Sub(v.timeStamp) < 1*time.Second {
	...
	case *MsgSendFrame:
		...
		pubsub.Publish(msg.Topic, event)
	case *actor.Stopping:
